---
name: "Laundry Platform Codebase Guide"
description: "Comprehensive instructions for working with the Laundry Pickup & Delivery platform codebase in antigravity IDE"
---

# Laundry Pickup & Delivery Platform - Antigravity Instructions

## Project Overview

This is a Next.js App Router-based laundry pickup and delivery platform with both admin and operator dashboards. The codebase uses TypeScript, React, TailwindCSS, shadcn/ui, and TanStack Query for state management and data fetching.
...
- `layouts/` - Layout components (e.g., app-sidebar)
...
#### Domain-Specific Components (`src/app/[domain]/component/`)

```
src/app/admin/component/
├── UserManagement.tsx
├── OrderStats.tsx
└── ReportGenerator.tsx
```
...
2. **Create schemas** → `src/schemas/[feature].ts`
3. **Create API hooks** → `src/hooks/use-[feature].ts`
4. **Create components** → `src/components/widgets/[Feature].tsx`
5. **Create pages** → `src/app/[route]/page.tsx`
...
4. Import helpers from `src/lib/api.ts`
